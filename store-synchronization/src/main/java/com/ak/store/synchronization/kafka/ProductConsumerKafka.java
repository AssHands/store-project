package com.ak.store.synchronization.kafka;

import com.ak.store.common.event.catalogue.ProductCreatedEvent;
import com.ak.store.common.event.catalogue.ProductDeletedEvent;
import com.ak.store.common.event.catalogue.ProductUpdatedEvent;
import com.ak.store.synchronization.elastic.ElasticService;
import lombok.RequiredArgsConstructor;
import org.springframework.kafka.annotation.KafkaListener;
import org.springframework.stereotype.Component;

@Component
@RequiredArgsConstructor
public class ProductConsumerKafka {
    private final ElasticService elasticService;

    @KafkaListener(topics = "product-created-events", groupId = "synchronization-product-group")
    public void handle(ProductCreatedEvent productCreatedEvent) {
        elasticService.createOneProduct(productCreatedEvent.getProductDocument());
    }

    @KafkaListener(topics = "product-updated-events", groupId = "synchronization-product-group")
    public void handle(ProductUpdatedEvent productUpdatedEvent) {
        elasticService.updateOneProduct(productUpdatedEvent.getProductDocument());
    }

    @KafkaListener(topics = "product-deleted-events", groupId = "synchronization-product-group")
    public void handle(ProductDeletedEvent productDeletedEvent) {
        elasticService.deleteOneProduct(productDeletedEvent.getId());
    }
}